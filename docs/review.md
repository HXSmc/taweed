...
>
> Written 2026-07-06 against `main` (`2d0e1bb`); updated 2026-07-08 against branch `ai-phase-4` (`e42821a`, pending merge to `main`) to cover **AI-4 (vision EOB/PDF extraction)**. Last spot-checked 2026-07-18 against `main` (`85737c9`) — branch-scoping (§1.8 walkthrough) and the full `/audit-workflow` 9-item queue (§2.9, §2.11) — see `docs/audit docs/audit.md` for the complete pass-by-pass record; this file only tracks what changes the onboarding-relevant claims here. Sources: `docs/handoff.md`, `docs/ai-deploy-readiness.md`, `docs/04_agentic_retrofit_plan.md`, `docs/blocker.md`, and a direct read of the code.
...
- **What to verify (branch selector):** picking a branch in the command bar (top of the page) narrows this page's numbers/charts to that branch only — e.g. at-risk SAR and the trend/Pareto/payer bars recompute; picking **All branches** reverts to the full-tenant figures. Analytics, Scrubber (Step 4), Appeals (Step 5), and Recovery (Step 6) now all have **real** branch filtering wired in (2026-07-18) — only Ingest still shows the same switcher chrome without filtering on it, so seeing no change there is expected, not a bug. An invalid or foreign `?branch=` value typed directly into the URL is silently ignored (falls back to All branches) rather than erroring — it can never widen scope beyond this tenant's own branches.
...
- **What to verify (branch selector):** picking a branch in the command bar (top of the page) narrows this table to that branch's claims only; picking **All branches** shows every branch again. Analytics, Appeals (Step 5), and Recovery (Step 6) also have **real** branch filtering now (2026-07-18) — only Ingest still shows the same switcher chrome without filtering on it, so seeing no change there is expected, not a bug. An invalid or foreign `?branch=` value typed directly into the URL is silently ignored (falls back to All branches) rather than erroring.
...
- **What to verify (branch selector, added 2026-07-18):** picking a branch in the command bar narrows the appealable-denials queue to that branch's claims only (live-verified: 100 → 79 for a real seeded branch); picking **All branches** restores the full queue. Live-verified EN + AR, both correctly narrowed and restored.
...
- **What to verify (branch selector, added 2026-07-18):** picking a branch narrows the ROI band (Recovered SAR, win rate, median days) AND the pipeline table below it TOGETHER — all three surfaces move in lockstep (live-verified: Recovered SAR 66,275→20,512, win rate 66.7%→67.1%, Won 25 of 160→25 of 53 for a real seeded branch); picking **All branches** restores full-tenant figures. The command-bar's own global "Recovered/At risk" money indicator (top of every page) stays UNFILTERED regardless of this page's branch selection — that's correct, intentional behavior, not a bug (it always reflects the layout's separate unfiltered money read). Live-verified EN + AR.
...
- **Unit tests (Vitest, ≈ pytest/JUnit):** pure logic, utilities, the AI code via a **fixture provider** (record/replay, so CI never calls the real API or needs a key). Baseline **1051 passing** as of 2026-07-18 (up from 769; grew through branch-scoping, the full `/audit-workflow` bug-fix/WCAG-fix/ponytail-cleanup passes, then net -2 from ponytail's dead-field-test removal — see `docs/audit docs/audit.md` for the pass-by-pass detail).
- **Integration tests:** run against a real Postgres, single-fork, **destructive**. They prove migrations, RLS enforcement, append-only privileges, and money reconciliation. Confirmed passing alongside the unit suite as of the 2026-07-18 audit-workflow item 1 push (combined unit+integration **1092/1092**); not independently re-run for every subsequent pass this session — several later passes were read-only or UI-only and correctly skipped the destructive integration re-run as disproportionate (see `docs/audit docs/audit.md`).
...
- ~~**Normalizer missing-amount → `0.00` (BLK-1 latent):** a missing adjudication amount currently becomes `"0.00"` instead of quarantining, which would *understate* denied/at-risk money.~~ **RESOLVED 2026-07-18** (`/audit-workflow` item 1, `docs/audit docs/bugs.md` pass #14): `packages/normalizer/src/normalize.ts`'s `total_amount`/`line_amount` now **throw** on a missing `claim.total?.value`/`item.net?.value` instead of silently folding to `"0.00"` — the real-data quarantine-vs-throw decision this bullet used to defer has been made (throw, not silently coerce). Regression tests: `packages/normalizer/test/normalize.test.ts` (2 new cases + 2 sibling fixtures updated to supply `total`).
- ~~**Accessibility — table row semantics:** the Scrubber uses `<tr role="button">` for clickable rows, which strips proper row semantics for screen readers. Flagged, deferred.~~ **RESOLVED** (WCAG AA accessibility audit, `docs/audit docs/a11y.md` #21, commit `516e72d`): native `<tr>`/`<td>` semantics restored (no `role`/`tabIndex` override), and the row's own mouse-only `onClick`/`cursor-pointer` was removed rather than papered over with a second `role="button"` — the nested claim-ID `<button>` (already `.focus-ring` + `aria-label`'d) is the row's one and only control. Regression-tested in `apps/web/test/scrubber-table.test.tsx` (7 tests: row/cell roles survive, the button is the sole interactive element, keyboard activation works, AR-locale aria-label) — reconfirmed green (7/7) 2026-07-10.
- **New 2026-07-18 (WCAG AA, `docs/audit docs/a11y.md` pass #18):** the branch-scoping feature's `tenant-switcher.tsx` dropdown had 3 real accessibility gaps, all fixed same day — branch scope dropping out of the accessible name below the `sm` breakpoint (`display:none` vs `sr-only`, same defect class as the already-fixed nav-rail finding), the single-select branch list using `menuitem`+`aria-current` instead of the WAI-ARIA APG `menuitemradio`+`aria-checked` pattern, and the selected item's text color failing AA contrast in dark theme (3.15:1, same root cause as the already-fixed `badge.tsx` finding). All 3 hub-live-verified via chrome-devtools (real accessible-name tree + real computed-style contrast), not just code inference.
