...
>
> **✅ Branch-count/insured-mix verification pass — 2026-07-19** (WebSearch/WebFetch, cross-checked against clinic sites, Vezeeta/Tebcan/Fresha listings, and insurer provider-network directories). Rows below marked **✅** are now independently confirmed (branch count and/or insured status); rows still marked (M)/(L) remain genuinely unconfirmed after this pass — not for lack of trying, the data simply isn't published anywhere findable. See row-specific notes for what changed.
...
| 5 | ⚠️ Quality Dental & Dermal / كواليتي — **LIKELY DUPLICATE OF #20** | Mecca, Jeddah, Madinah, Tabuk, Taif *(✅ corrected — not Riyadh/Tabuk as previously listed)* | Dental + derma/aesthetics | ~10 ✅ | [vezeeta listing](https://saudi.vezeeta.com/en/clinic/quality-dental-clinics) | H | 5 — **2026-07-19: this Vezeeta listing's phone number (+966 9200 04864) matches Aljawdah Clinics' (#20) listing exactly — same entity under two directory names.** Do not count both rows; keep #20 (has a first-party site), drop this row before CRM load. Insurance: unconfirmed (Vezeeta's insurer filter is platform-wide, not clinic-specific). |
| 6 | Roaya Eye Center / رؤية للعيون | Riyadh only ✅ *(corrected — Jeddah/Dammam/Buraidah NOT corroborated by the official site)* | Ophthalmology | 1 ✅ | [roayaeyecenter.com](https://roayaeyecenter.com) | H | 4 — **2026-07-19: official site's own contact page lists only one branch (Riyadh, King Fahad Rd).** Downgraded from 5 (multi-city was an unverified aggregator claim). Insurance ✅ CONFIRMED strong — Vezeeta booking page lists 40+ named insurers (Bupa Gold, Tawuniya Premium A, MedGulf VIP, etc.). Also note: **≥4 similarly-named but distinct KSA eye centers exist** (Roayah Vision Correction, Roaya Eye & Retina, Al Ro'ya, Royaeyeclinic) — confirm outreach targets roayaeyecenter.com specifically, not a namesake. |
| 7 | Ryan Clinics / ريان | Dammam, Jeddah, Riyadh, Makkah ✅ | Polyclinic | 4 cities ✅ | [ryanclinicdammam.com](https://ryanclinicdammam.com) | H | 5 — **2026-07-19: confirmed, own branded site per city.** Insurance ✅ CONFIRMED strong — explicit "cashless treatment" with 14 named insurers (Bupa Arabia, Tawuniya, GIG, SAICO, Walaa, GlobeMed, Cigna, NextCare, etc.). One of the two best-confirmed insured-mix targets in this list. |
...
| 9 | Dora Clinics / دورا | Riyadh only ✅ | Derma + laser + injectables | 4 ✅ | [doraclinic.sa](https://doraclinic.sa/en/) | H | 3 — **2026-07-19: 4 branches confirmed, all Riyadh, none outside.** Insurance: no evidence found anywhere (site or search) — pure derma/laser/injectables reads as cash-pay under standard KSA cosmetic-exclusion policies. Downgraded from 4 to 3 on that basis. |
...
| 11 | Cosmetic Clinic Jeddah / كوزمتك كلينك | Jeddah, Riyadh, Dammam ✅ | Aesthetics/plastic, derma, dental | 3 cities ✅ | [cosmeticlinicjeddah.com](https://cosmeticlinicjeddah.com/) | H | 4 — cities confirmed on own about-us page; cash-pay stance unchanged (no insurer mentions) |
...
| 15 | Derma Clinics / ديرما | Riyadh, Jeddah ✅ | Derma + laser + plastic + hair | 2 ✅ (possibly more Riyadh addresses, unconfirmed) | [derma-clinic.com](https://derma-clinic.com/) | H | 4 — 2 branches solidly confirmed (Riyadh + Jeddah via independent Fresha listings, own site blocked fetch). Insurance: genuinely unconfirmed — mixes medical derma+dental (insurable) with cosmetic/plastic (likely cash); worth a direct call rather than assuming either way. |
| 16 | Al-Zallal / الزلال | Riyadh (×4), Al-Khobar ✅ | Derma + cosmetic + laser (+dental) | 5 ✅ (upgraded from "2+") | [drazallal.net](https://drazallal.net/) | H | 4 — 5 branches confirmed via Tebcan center listings. Insurance still unconfirmed either way (Tebcan's "no insurances listed" for one branch reflects directory gaps, not proof of cash-only, given the medical-dermatology/dental mix). |
...
| 18 | Al Subhi Medical Center / الصبحي | Riyadh only ✅ | Polyclinic (+dental/derma) | 1–2 ✅ (downgraded — "25 clinics/14 divisions" refers to specialty departments within one complex, not separate branches) | [asmc.med.sa](https://asmc.med.sa) | H | 4 — **2026-07-19: insurance ✅ CONFIRMED via THREE independent insurer provider-network listings** (Bupa Arabia, Tawuniya MPN, GIG Network E) — the single best-confirmed insured-mix signal in this whole list, even though branch count is smaller than assumed. |
| 19 | Taiba Dental / طيبة | Riyadh | Dental | 2–3 | [taiba-clinics.com](https://taiba-clinics.com/en/about-us/) | H | 3 — dentist-led but small |
| 20 | Aljawdah Clinics / الجودة | Mecca, Jeddah, Madinah, Tabuk, Taif ✅ *(corrected — NOT Jeddah-only as previously listed)* | Dental + derma + ortho | 10 ✅ | [aljawdahclinics.com](https://aljawdahclinics.com/) | H | 4 — **2026-07-19: 10 branches across 5 cities confirmed directly from the live site (independently cross-checked twice).** See #5 above — same entity, listed there under its Vezeeta directory name; drop #5, keep this row. Insurance still unconfirmed. |
| 21 | ⚠️ Loran Dental / لوران — **downgrade, cash-only confirmed** | Riyadh | Dental | 1 ✅ | [loranclinics.com](https://loranclinics.com/) | H | 2 — **2026-07-19: confirmed single branch AND confirmed cash-pay only** (Tebcan listing explicitly states "No insurances listed... pay in the medical center"). Downgraded from 3 — fails the insured-mix ICP criterion outright, not just small. |
...
| 23 | Royal Clinic / رويال كلينك | Riyadh, Jeddah ✅ | Cosmetic + laser + derma (+dental) | 2 ✅ | [royalclinicsaudia.com](https://www.royalclinicsaudia.com/en-sa/) | H | 3 — branch count confirmed (Riyadh Al Hamra + Jeddah Qouraish/As Salamah); cash-heavy stance unchanged, no insurer evidence found |
| 24 | ⚠️ Novello Clinics KSA / نوفيلو — **branch count corrected down** | Riyadh (Al Malqa) confirmed; Jeddah presence unconfirmed | Laser + aesthetic + derma | 1 confirmed ✅, rest unconfirmed (M) | [sa.novelloclinics.com](https://sa.novelloclinics.com/en/) | M | 2 — **2026-07-19: the "10 branches" figure some sources attach to this name actually belongs to the separate Egyptian entity (novelloclinics.com, Cairo/Giza) — do not reuse it for the KSA chain.** Saudi site only clearly documents one branch (Al Malqa, Riyadh). Downgraded from 3 to 2 pending real confirmation. Cash-pay likely ("Installments at Cash Price" language, no insurer mentions). |
...
| 26 | Shifa Jeddah Polyclinic / شفاء جدة | Jeddah | Polyclinic (11 depts) | 2 ✅ | [shifajeddah.com](https://shifajeddah.com) | H | 4 — **2026-07-19: insurance ✅ CONFIRMED strong** — own site's Insurance Coverage page lists Bupa, TCS, Gulf Union, Saico, Malath, Al Etihad, Nextcare, plus corporate clients. Upgraded from 3 to 4 on that basis. |
...
| 29 | Badr al Tamam Polyclinic / بدر التمام | Jeddah | Polyclinic (9 spec.+ER) | 1 ✅ (confirmed, unchanged) | [badraltamam.com](https://badraltamam.com) | H | 2 — single-site confirmed via own contact page; insurance still unconfirmed (no evidence found despite having an ER, which typically carries insurance — worth a direct call) |
| 30 | Kaya Skin Clinic KSA | Riyadh (×2), Jeddah (×1) confirmed, likely more (M) | Skin / derma / laser | ≥3 ✅, exact total unconfirmed | [kayaskinclinic.com/ksa](https://www.kayaskinclinic.com/ksa/en) | H | 2 — corporate (not physician-owned) chain within a larger 20–23-clinic UAE/Oman/KSA network; cash-pay likely (aesthetic focus, no insurer evidence) |
| 31 | Dentalia Care / دنتاليا كير | Jeddah | Dental | 2 ✅ (upgraded from "2+, low confidence") | [dentaliaclinics.com.sa](https://dentaliaclinics.com.sa/) | H | 3 — **2026-07-19: 2 branches confirmed (Al-Tahlia + Al-Shati) via own contact page.** Insurance likely but not fully confirmed (Vezeeta lists per-doctor insurance coverage, "accredited by insurance companies" per search, no specific insurer named on own site). |
...
- **Cash-pay caution — now includes confirmed cases, not just suspected:** rows 9, 11, 21, 23, 24, 30 (cosmetic/laser/derma-only) skew cash-pay → lower denial exposure → deprioritize unless they confirm insured volume. **Row 21 (Loran) is now a confirmed cash-only single-site — consider dropping outright**, it fails the ICP on both branch count and insured mix.
- **Duplicate to resolve before CRM load:** rows 5 and 20 are the same entity (Aljawdah Clinics) under two directory listings — keep #20, drop #5.
