...
> **Cloud / KSA region (RESEARCH):** PDPL data sovereignty means we host **in-Kingdom**. As of research date, the only hyperscaler with an in-Kingdom region offering local data residency is **AWS (me-central-2 / "Saudi Arabia" region)** — announced with a >$5.3B investment in March 2024 ([AWS, via DCD](https://www.datacenterdynamics.com/en/news/microsoft-to-launch-saudi-arabia-east-region-in-q4-2026/)); **Microsoft Azure "Saudi Arabia East"** (Eastern Province, 3 AZs) is a **future Q4 2026 launch, not yet live** ([DCD, Feb 2026](https://www.datacenterdynamics.com/en/news/microsoft-to-launch-saudi-arabia-east-region-in-q4-2026/)). No Google Cloud or Oracle KSA in-Kingdom-region data was confirmed — **UNVERIFIED (M)** (note: Insta by Practo hosts its KSA product "on Oracle Cloud," but that does **not** confirm an Oracle in-Kingdom region — verify separately). ⚠️ **AWS Saudi region is announced/planned for 2026 and is NOT yet Generally Available** per AWS's own [press release (Mar 2024)](https://press.aboutamazon.com/2024/3/aws-to-launch-an-infrastructure-region-in-the-kingdom-of-saudi-arabia) ("launch in 2026," >$5.3B, in-country residency) and the [AWS Regions & AZs page](https://aws.amazon.com/about-aws/global-infrastructure/regions_az/) (listed announced/planned, not GA). The earlier "live Jan 2026" claim was refuted. **Plan for a launch-window dependency; confirm live GA with AWS before committing** (§10).
>
> **Decision:** target **AWS in-Kingdom region** for MVP (only confirmed in-Kingdom option). Keep infra cloud-portable (Terraform, containers, Postgres, S3-compatible) so Azure KSA (Q4 2026) stays a fallback. Hosting cost: **§10**.
...
### Onboarding path (directional — confirm with CHI/NPHIES; sources are secondary, M/L)
A new provider/vendor reportedly progresses through **training → clinical readiness → technical readiness → integration → certification** before transacting on the **NPHIES production environment** ([NPHIES Readiness & Activation Guide, secondary/unreliable — verify](https://www.scribd.com/document/719878262/nphies-Readiness-Activation-Guide-V1-0-English)). Each onboarding organization is reportedly issued a **PKI credential used to authenticate to NPHIES** ([getcirrus blog, M — verify](https://www.getcirrus.com/en-blog/nphies-for-hospitals-and-clinics-in-saudi-arabia)). ⚠️ **The exact PKI issuance process, sandbox/conformance test procedure, and certification steps were NOT pinned to primary sources this pass — treat as UNVERIFIED (M/L); confirm via the NPHIES portal / CHI onboarding team before planning Phase 2.**
...
- **Clearinghouse:** route via an existing connected intermediary (e.g., **Waseel**, a long-standing KSA clearinghouse — [waseel.com](https://waseel.com/rcm/)). Faster to live, less PKI/conformance lift, at a per-transaction cost + dependency.
- **Recommendation:** **Phase 1 = neither** (file ingestion, zero NPHIES creds — fastest value). **Phase 2 = start via clearinghouse** to reach live data quickly, then evaluate **direct** once volume justifies owning the integration. *Decision gate: confirmed onboarding burden + clearinghouse pricing (open question, File 1 §9).*
...
- **Region availability (verified, see §4):** **AWS Saudi Arabia region** — **CONFIRMED (H, 3-0)** as **announced/planned for launch in 2026, NOT yet Generally Available** (no GA date), per AWS's own [Regions & AZs page](https://aws.amazon.com/about-aws/global-infrastructure/regions_az/) + [press release](https://press.aboutamazon.com/2024/3/aws-to-launch-an-infrastructure-region-in-the-kingdom-of-saudi-arabia). **Azure Saudi Arabia East** = Q4 2026 (future). Google/Oracle KSA in-Kingdom regions **UNVERIFIED (M)**. ⚠️ **Launch-window dependency:** if you need in-Kingdom hosting before AWS/Azure go GA, options narrow — check current GA status at build time and consider a KSA-based/local provider as interim.
...
|---|---|---|---|
| [NPHIES IG package](https://portal.nphies.sa/ig/index.html) | NPHIES/CCHI-governed (⚠️ confirm reuse terms) | **Single highest-leverage artifact.** Download the IG NPM package → drives validation, conformance, synthetic-bundle generation. StructureDefinitions/ValueSets/profiles for Claim/ClaimResponse/eligibility. | **H** |
...
| [Fadil369/NPHIES](https://github.com/Fadil369/NPHIES) | MIT | ⚠️ early-stage / low-star — **confirm it exists + what's implemented** before treating as a reference. | M (verify) |
...
> ⚠️ **Verify before citing:** arXiv:2602.05374 ("Feb 2026") has an implausible/future ID — confirm it exists; the Binghamton three-stage-denial thesis URL is malformed — find a stable citation; the two Najran studies' exact error/financial figures must be read from source PDFs (do **not** reuse the refuted 26.8%/9.9% vendor numbers without confirming).

### 12.3 Recommended starter stack + critical gaps
**Opinionated MVP pick:** **Medplum (`@medplum/fhirtypes` + `validateResource` + Project tenancy)** + **fhirpath.js** for FHIR-native checks + **json-rules-engine** (→ ZEN when compliance staff author rules) + **Synthea** for test data + **`validator_cli.jar`/Matchbox** as a CI conformance gate loaded with the **NPHIES IG**. All permissive (Apache/MIT/ISC) except the NPHIES IG (governed — confirm bundling terms).
